...
def _lobby_spawn_positions(member_ids: list[int], *, grid: int = CWAR_GRID_SIZE) -> dict[int, tuple[int, int]]:
    positions: dict[int, tuple[int, int]] = {}
    spawn_cols = (0, 1, 2)
    for index, pid in enumerate(member_ids):
        col = spawn_cols[index % len(spawn_cols)]
        row = min(grid - 1, index // len(spawn_cols))
        positions[pid] = (col, row)
    return positions


def render_war_lobby_preview_frame(
    storage: Storage,
    *,
    location_name: str,
    member_ids: list[int],
    ...
    """Карта ожидания в военном лобби: расстановка бойцов и точка штурма."""
    grid = CWAR_GRID_SIZE
    cell = 64
    grid_px = grid * cell
    margin = 16
    ...
    control_point = (4, 4)
    base_cover = [(x, y) for y in range(grid) for x in range(grid - 3, grid)]
    spawn_positions = _lobby_spawn_positions(member_ids, grid=grid)
    ...
    y = margin + 12
    draw.text((pl + 10, y), f"Лобби {location_name[:18]}", fill=(240, 240, 240), font=small)
    y += 22
    draw.text((pl + 10, y), f"Бойцов: {len(member_ids)}", fill=(180, 180, 180), font=small)
    y += 15
    draw.text((pl + 10, y), "Жёлтая рамка — лидер", fill=(200, 200, 120), font=small)
    y += 15
    for pid in member_ids[:6]:
        ch = storage.get_character(pid, refresh_energy=False)
        name = (ch.nickname if ch else str(pid))[:24]
        mark = " 👑" if pid == leader_id else ""
        if pid == viewer_id:
            mark += " (ты)"
        draw.text((pl + 10, y), f"{name}{mark}"[:38], fill=(180, 180, 180), font=small)
        y += 15
