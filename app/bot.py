...
def _build_info_text(player: Character) -> str:
    return (
        ...
        "  В коопе: 1 аптечка/боец, эвакуация раненого (рядом + тащить на старт). "
        ...
        "• 🏚 Рейды (тактика 9×9, 2–5 бойцов, свои или союзники): логово, склад/гараж врага; "
        ...
        "  Провал логова: −110 RU, −8 рейтинга; склад/гараж: −90 RU, −6 рейтинга.\n"
        ...
        "(нужен детектор).\n"
        f"• ⚔️ Война: нейтральные — соло (+{NCAP_SUCCESS_PAY_RU} RU, −18 энергии) или лобби от 5; "
        f"занятые точки — только лобби (тактический штурм 9×9, −{WAR_LOBBY_ENERGY_COST} энергии, "
        f"хост +{WAR_SUCCESS_PAY_RU} RU, союзники +{WAR_ALLY_SUCCESS_PAY_RU} RU).\n"
        ...
        "только при добровольном выходе («Покинуть»), если зачистил ≥1 волну.\n"
...
async def war_scenario_section_callback(callback: CallbackQuery) -> None:
    ...
    explainer = (
        "Правила войны:\n"
        f"• Нейтральные точки — соло-захват на сетке 6×6 "
        f"(«🎯 Захват нейтральных точек», +{NCAP_SUCCESS_PAY_RU} RU, −18 энергии) "
        "или через лобби от 5 бойцов.\n"
        "• Занятые точки и базы — только военное лобби (минимум 5 бойцов), "
        f"тактический штурм на сетке 9×9 (−{WAR_LOBBY_ENERGY_COST} энергии каждому).\n"
        f"• Награды лобби: хост +{WAR_SUCCESS_PAY_RU} RU, союзники +{WAR_ALLY_SUCCESS_PAY_RU} RU "
        "(только выжившим).\n"
        "• Лидер группировки, стоя на своей точке, может передать её союзнику "
        "(«🎁 Передача точки союзнику»).\n"
        "• Рейды на логова — в «🪖 Рейды» (2–5 бойцов, свои или союзники); "
        "успех: 1400 + 180×выживших RU в казну; "
        "на склад/гараж врага — только без союза.\n"
        "• В рейде: 1 аптечка из инвентаря, «💊 Поднять» раненого на соседней клетке; "
        ...
        "• Укрепление базы: лидер, 10000 RU из казны, +1 защитник и урон за уровень в штурме.\n"
